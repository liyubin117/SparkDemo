...
//      Map[String, Int]("test" -> 1), //日志在Kafka中的topic及其分区，键是topic名，值是topic的分区数
...
//      .map(x => x._2.split("\\|~\\|", -1))  //日志以|~|为分隔符
     ...
     */
    val stream = KafkaUtils.createDirectStream[String, String, StringDecoder, StringDecoder](ssc, Map("metadata.broker.list" -> "spark:9092"), Set("test"))
                  .map(x => x._2.split("\\|~\\|", -1))  //日志以|~|为分隔符
